...
public class SmellDetector {
    ...
    private static HashSet<SmellLocation> usingEval = new HashSet<>();    // keepig using eval code line number
    private static HashSet<SmellLocation> usingInnerHTML = new HashSet<>();
    private static HashSet<SmellLocation> usingOctal = new HashSet<>();    // keeping unreachable code line number
    ...
    public void analyseAstNode() {
        ...
        if (ASTDepth == assignmentNodeDepth + 1) {
            ...
            if (ASTNodeName.equals("Name"))
                ...
                analyseBlock();
            else if (ASTNodeName.equals("ExpressionStatement"))
    ...
    private void analyseExpressionStatement() {
        System.out.println("Analysing Expression Statement ###############");
        analyseUsingEval();
        analyseUsingInnerHTML();
        analyseOctal();
    }

    private void analyseUsingEval() {
        String source = this.ASTNode.toSource();
        System.out.println("Source " + source);
        if (source.contains("eval")) {
            String sourceName = ASTNode.getAstRoot().getSourceName();
            SmellLocation sl = new SmellLocation("using Eval", sourceName, ASTNode.getLineno());
            usingEval.add(sl);
        }
    }

    private void analyseUsingInnerHTML() {
        String source = this.ASTNode.toSource();
        System.out.println("Source " + source);
        if (source.contains("innerHTML")) {
//            String sourceName =  AstNode.getAstRoot().sourceName;
            String sourceName = ASTNode.getAstRoot().getSourceName();
            SmellLocation sl = new SmellLocation("using innerHTML", sourceName, ASTNode.getLineno());
            usingInnerHTML.add(sl);
        }
    }


    private void analyseOctal() {
        System.out.println("Analysing Octal ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
        AstNode ast = this.ASTNode;
        String value;
        try {
            ExpressionStatement ex = ((ExpressionStatement) ast);
            value = ((Assignment) ex.getExpression()).getRight().toSource();
        } catch (ClassCastException e) {
            System.out.println("Error casting ExpressionStatement");
            return;
        }
        if (value.startsWith("0")) {
            //Octal value
            String sourceName = ASTNode.getAstRoot().getSourceName();
            SmellLocation sl = new SmellLocation("using Octal", sourceName, ASTNode.getLineno());
            usingOctal.add(sl);
        }
    ...
    public static void writeReportTofile() {
        ...
        try {
            ...
            out.write("********** Using Eval **********\n");
            out.write(reportSmell(usingEval));

            out.write("********** Using innerHTML **********\n");
            out.write(reportSmell(usingInnerHTML));

            out.write("********** Using Octal **********\n");
            out.write(reportSmell(usingOctal));
